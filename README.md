...
This repository contains a PostgreSQL project analyzing Spotify track data. It includes a single-table dataset and demonstrates data exploration, aggregation, ranking, window functions, and automated engagement calculations using SQL functions and stored procedures.
...
3. [Analytics Queries](#analytics-queries)  
    - [1. Top Tracks by Engagement](#1-top-tracks-by-engagement)  
    - [2. Comments by Licensed Tracks](#2-comments-by-licensed-tracks)  
    - [3. Album Averages by Platform](#3-album-averages-by-platform)  
    - [4. Singles with Over 1M Streams](#4-singles-with-over-1m-streams)  
    - [5. Total Tracks per Artist](#5-total-tracks-per-artist)  
    - [6. Album Danceability](#6-album-danceability)  
    - [7. Top 5 Artists by Average Engagement](#7-top-5-artists-by-average-engagement)  
    - [8. Official Video Likes and Views](#8-official-video-likes-and-views)  
    - [9. Album Total Views](#9-album-total-views)  
    - [10. Tracks More Streamed on Spotify than YouTube](#10-tracks-more-streamed-on-spotify-than-youtube)  
    - [11. Top 3 Tracks per Artist by Views](#11-top-3-tracks-per-artist-by-views)  
    - [12. Tracks Above Average Liveness](#12-tracks-above-average-liveness)  
    - [13. Energy Difference per Album](#13-energy-difference-per-album)  
4. [SQL Functions](#sql-functions)  
    - [Engagement Tier Classification](#engagement-tier-classification)  
5. [Stored Procedures](#stored-procedures)  
    - [Update Track Engagement Ratio](#update-track-engagement-ratio)  
...
This project analyzes a Spotify dataset containing track metadata and streaming metrics. It focuses on identifying trends in audience engagement, platform-specific performance, and artist-level insights. The project includes:

- Single-table database setup  
- Analytical queries for tracks, albums, and artists  
- Ranking, aggregation, and filtering logic  
- SQL functions to classify engagement tiers  
- Stored procedures to automate engagement calculations  

All tasks are designed to extract actionable insights from music streaming data using SQL.
...
## Analytics Queries
Finding tracks with the highest audience engagement, where engagement is a combination of likes, comments, and streams.
```sql
SELECT
    track,
    artist,
...
LIMIT 20;
```
## 2. Comments by Licensed Tracks
Getting the total number of comments for tracks depending on the license
...
FROM spotify;
```
## 3. Album Averages by Platform
Calculating average danceability, energy, and valence for each album, separated by platform (most_played_on) for albums that have at least 3 tracks
...
```
## 4. Singles with Over 1M Streams
Finding all tracks that belong to the album type single with more than 1 million streams
...
ORDER BY 2 DESC
```
7. Top 5 Artists by Average Engagement
Finding the top 5 artists with the highest average engagement (likes+comments)/streams WHERE:
...
```
## 8. Official Video Likes and Views
Listing all the tracks along with their views and likes depending where official_video = True
...
```
## 9. Album Total Views
For each album, calculating the total views and all associated tracks.
...
ORDER BY 3 DESC
```
## 10. Tracks More Streamed on Spotify than YouTube
Retrieving the track names along with their streams based where streams on spotify are greater than on youtube
...
ORDER BY 3 DESC
```
## 11. Top 3 Tracks per Artist by Views
Finding top 3 most viewed tracks for each artist
...
Writing a query to find tracks where the liveness score is above the average
